...
\begin{itemize}
\item \alert{Diffusion distance}: Euclidian distance in the eigenvector space.

\item \alert{Diffusion coefficients}: Eigenvalues in the direction of their corresponding eigenvectors

\item Assumption: Eigenvalues drop to a noise level after the first few components
\end{itemize}
\end{frame}
...
\item Choose $\sigma$ such that $d(\sigma)\rangle_x$ is maximized
